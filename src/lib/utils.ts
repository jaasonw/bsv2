...
export function createTable(
  ...
  df = df.push([
    "total",
    ...people.map((name) => getPartialTotal(name, tip, tax, tipAsProportion, people, items).toFixed(2)),
...
function getPartialTotal(person: string, tip: number, tax: number, tipAsProportion: boolean, people: string[], items: Item[]) {
  let partialTip = tipAsProportion
    ? getPartialAmount(person, tip, items)
    : tip / people.length;

  return (
    getPartialSubtotal(person, items) + partialTip + getPartialAmount(person, tax, items)
  );
}
...
export function validateTotals(people: string[], tip: number, tax: number, tipAsProportion: boolean, items: Item[]) {
  // sum all partial subtotals
  let calculated = people.reduce((acc, person) => {
    return acc + getPartialTotal(person, tip, tax, tipAsProportion, people, items)
  }, 0);
